...
class ACL(object):
    ...
    def __init__(self, model, args, network, 
                extradata=None, gl_start_predict=0, gl_prob=0.0,
                extra_data_name='cub', grad_learner=None):
        self.args=args
        ...
        self.gl_start_predict = gl_start_predict
        self.gl_prob = gl_prob
        self.extra_data_name = extra_data_name
        self.grad_learner = grad_learner
        self.optimizer_G=self.get_G_optimizer(0)
        # self.optimizer_grad_learner=torch.optim.SGD(self.grad_learner.parameters(), momentum=self.args.mom,
    ...
    def get_G_optimizer(self, task_id, e_lr=None):
        if e_lr is None: e_lr=self.e_lr[task_id]
        optimizer_G=torch.optim.SGD(self.grad_learner.parameters(), momentum=self.args.mom,
                                    weight_decay=self.args.e_wd, lr=e_lr)
        return optimizer_G
    ...
    def train(self, task_id, dataset):
        ...
        self.optimizer_G=self.get_G_optimizer(task_id, e_lr)

        tr_ep_stats = [AverageMeter() for x in range(1)]
        val_loss = AverageMeter()
        for e in range(self.nepochs):

            # Train
            clock0=time.time()
            train_stats = self.train_epoch(dataset['train'], task_id)
            clock1=time.time()

            for i in range(len(tr_ep_stats)):
                tr_ep_stats[i].update(train_stats[i])
            ...
            val_loss.update(valid_res['loss_tot'])
        ...
        stats = []
        for i in range(len(tr_ep_stats)):
            stats.append(tr_ep_stats[i].avg)
        stats += [val_loss.avg]
        return stats
    ...
    def train_epoch(self, train_loader, task_id):
        ...
        loss_grad = AverageMeter()
        grle_cos = AverageMeter()
        grpr_cos = AverageMeter()
        grle_time = AverageMeter()
        grpr_time = AverageMeter()
        ...
        for data, target, tt, td in train_loader:
            ...
            for s_step in range(self.s_steps):
                ...
                pred_grad = self.grad_learner(output_copy.data)
                cur_lr = self.optimizer_S.param_groups[0]['lr']
                nml_pred_grad = self.grad_learner.gl_scale*pred_grad / \
                            torch.norm(pred_grad, dim=1, keepdim=True) * \
                            torch.norm(intermediate_grads['output'], dim=1, keepdim=True).mean(dim=0, keepdim=True)
                loss_in_grad = self.task_loss(output_copy-cur_lr*nml_pred_grad, y)
                self.optimizer_G.zero_grad()
                loss_in_grad = self.grad_learner.gl_loss_scale*loss_in_grad
                loss_in_grad.backward()
                self.optimizer_G.step()
                time_grle = time.time() - start_grle
                loss_tr.update(task_loss.item())
                loss_grad.update(loss_in_grad.item()/self.grad_learner.gl_loss_scale)
                grle_time.update(time_grle)
                #--------------------------
                ...
                #--------------------------
                x_extra = None
                if random.uniform(0, 1) < self.gl_prob:
                    if self.extra_data_name == 'cub':
                        x_extra, _, _, _ = next(self.extradata)
                    else:
                        x_extra, _ = next(self.extradata)
                    x_extra = x_extra.to(device=self.device)
                if x_extra is not None and x_extra.size(0) == x.size(0) and i >= self.gl_start_predict:
                    output_grad = intermediate_grads['output']
                    start_grpr = time.time()
                    extra_outputs=self.model(x_extra, x_task_module, tt, task_id)
                    with torch.no_grad():
                        pred_extra_grad = self.grad_learner(extra_outputs)
                    mean_pred_extra_grad = pred_extra_grad.data.mean(dim=0, keepdim=True)
                    grad_for_update = self.grad_learner.gl_scale*pred_extra_grad / \
                ...
                tmp_train_loss.update(total_loss.item())
            train_loss.update(tmp_train_loss.avg, x.size(0))
