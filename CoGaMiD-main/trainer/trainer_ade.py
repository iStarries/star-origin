...
class Trainer_incremental(Trainer_base):
    ...
    def _train_epoch(self, epoch):
        ...
        if epoch == 1:
            ...
            self.per_iter_prev_number = torch.clamp(self.per_iter_prev_number, min=1)
            ...
            if self.task_info['setting'] == 'overlap':
                ...
                # 过去背景中，有一部分是现在类别
                tot_numbers[0] = (self.pred_numbers[0] * 0.5).to(torch.int) + self.prev_bg_number
            else:
                # 现在背景和过去背景无重复
                # 过去背景中也不存在现在类别
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=self.config['use_amp']):
                ...
                for cls in range(0, self.per_iter_prev_number.shape[0]):
                    per_cls_fake_features = self.prev_fes_model[cls].cpu().sample(int(self.per_iter_prev_number[cls]))[
                        0].to(self.device)
                    per_cls_fake_features = per_cls_fake_features.permute(1, 0).unsqueeze(0).unsqueeze(3)

                    prev_gmm_prototype.append(
                        (self.prev_fes_model[cls].mu.squeeze(0) * self.prev_fes_model[cls].pi.squeeze(0)).sum(
                            dim=0).unsqueeze(0))
                ...
                negtive_mask = torch.logical_and(data['label'] == 0, (
                            logit_old.max(dim=1)[0] > logit[:, -self.n_new_classes:].detach().max(dim=1)[0]).squeeze(
                    1)).unsqueeze(1)
                ...
                loss_mbce_ori = self.BCELoss(
                    logit[:, -self.n_new_classes:],  # [N, |Ct|, H, W]
                    data['label'],fake_features,                # [N, H, W]
                ).mean(dim=[0, 2, 3])
                ...
                loss_pkd = self.PKDLoss(features, features_old, pseudo_label_region.to(torch.float32),
                                        self.prev_fes_model, self.device)
