    ...
    def _test(self, epoch=None):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.test_loader):
                data['image'], data['label'] = data['image'].to(self.device), data['label'].to(self.device)
                target = data['label'].cpu().numpy()
                logit, features, _ = self.model(data['image'],ret_intermediate=True)
...
class Trainer_incremental(Trainer_base):
    ...
    def __init__(
        ...
        self.train_metrics = MetricTracker(
            'loss', 'loss_mbce', 'loss_pkd', 'loss_cont','loss_uncer',
        ...
        self.PKDLoss = PKDLoss(n_old_classes=self.n_old_classes + 1, n_new_classes=self.n_new_classes)


        self.UnCertainloss = nn.MSELoss(reduction='mean')
        self.ContLoss = ContLoss(n_old_classes=self.n_old_classes + 1, n_new_classes=self.n_new_classes)
        if config['data_loader']['args']['task']['step'] >= 2:
            prev_info_path = \
                str(config.save_dir)[:-len(str(config.save_dir).split('_')[-1])] + \
                str(config['data_loader']['args']['task']['step'] - 1) + \
                "/prototypes-epoch{}.pth".format(config['trainer']['epochs_incre'])
        else:
            prev_info_path = \
                str(config.save_dir)[:-len(str(config.save_dir).split('_')[-1])] + \
                str(config['data_loader']['args']['task']['step'] - 1) + \
        ...
        self.prev_fes_model = prev_info['fes_model']
    ...
    def _train_epoch(self, epoch):
        ...
        if epoch == 1:
            ...
            if self.task_info['setting'] == 'overlap':
                ...
                self.per_iter_prev_number[self.per_iter_prev_number < 1] = 1
            else:
                self.per_iter_prev_number = \
            ...
            if self.task_info['setting'] == 'overlap':
                if torch.sort(self.pred_numbers[1:] / self.prev_numbers[1:])[0][-1] < 1:
                    self.extra_bg_ratio = ((self.prev_bg_number - (
                        torch.sort(self.pred_numbers[1:] / self.prev_numbers[1:])[0][-1]) * self.numbers[1:].sum()) /
                                           self.pred_numbers[0])
                else:
                    self.extra_bg_ratio = ((self.prev_bg_number - self.numbers[1:].sum()) / self.pred_numbers[0])
            else:
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=self.config['use_amp']):
                ...
                for cls in range(0, self.per_iter_prev_number.shape[0]):
                    ...
                    per_cls_fake_features = self.prev_fes_model[cls].cpu().sample(int(self.per_iter_prev_number[cls]))[0].to(self.device)
                    per_cls_fake_features = per_cls_fake_features.permute(1,0).unsqueeze(0).unsqueeze(3)

                    prev_gmm_prototype.append((self.prev_fes_model[cls].mu.squeeze(0)*self.prev_fes_model[cls].pi.squeeze(0)).sum(dim=0).unsqueeze(0))
                ...
                prev_gmm_prototype = torch.cat(prev_gmm_prototype,dim=0)
                ...
                negtive_mask = torch.logical_and(data['label']==0,(logit_old.max(dim=1)[0]>logit[:, -self.n_new_classes:].detach().max(dim=1)[0]).squeeze(1)).unsqueeze(1)
                pseudo_label_region = pseudo_label_region*negtive_mask
                ...
                loss_pkd = self.PKDLoss(features, features_old, pseudo_label_region.to(torch.float32),self.prev_fes_model, self.device)
                uncer_map = calculate_certainty(torch.sigmoid(logit))

                fg = (data['label'] == (torch.argmax(logit, dim=1) + 1)) | (
                            torch.max(torch.sigmoid(logit), dim=1)[0] > 0.7)

                mask_fg = torch.where(fg, torch.zeros_like(uncer_map.squeeze(1)),
                                      torch.ones_like(uncer_map.squeeze(1))).to(torch.float32)
                loss_uncer = self.UnCertainloss((1 - uncer_map.squeeze(1)) * mask_fg,
                                                torch.zeros_like(uncer_map.squeeze(1)).to(self.device))

                loss_cont = self.ContLoss(
                    features[-1], logit, data['label'], self.n_new_classes,prev_gmm_prototype.to(self.device))


                loss = self.config['hyperparameter']['mbce'] * loss_mbce.sum() \
                       + self.config['hyperparameter']['pkd'] * loss_pkd.sum() \
                       + self.config['hyperparameter']['cont'] * loss_cont \
                       + self.config['hyperparameter']['uncer'] * loss_uncer
            ...
            self.train_metrics.update('loss_uncer', loss_uncer.item() * self.config['hyperparameter']['uncer'])
    ...
    def compute_pred_number(self):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            with torch.no_grad():
                data['image'], data['label'] = data['image'].to(self.device), data['label'].to(self.device)
                logit_old, features, _ = self.model_old(data['image'], ret_intermediate=True)
