...
def main(config):
    ...
    if config['multiprocessing_distributed']:
        ...
        p = mp.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, config))
    else:
...
def main_worker(gpu, ngpus_per_node, config):
    ...
    def _get_prev_step_checkpoint(prev_dir, trainer_cfg, prev_step, best_source):
        if best_source:
            best_path = None
            best_miou = -float('inf')
            ...
            for ckpt in prev_dir.glob(pattern):
                stem = ckpt.stem
                try:
                    miou_val = float(stem.split("miou")[-1])
                except Exception:
                    continue
        ...
        final_epoch = trainer_cfg['epochs'] if prev_step == 0 else trainer_cfg['epochs_incre']
        return prev_dir / f"checkpoint-epoch{final_epoch}.pth"
    ...
    # Create Dataloader
    dataset = config.init_obj('data_loader', module_data, concat_all_val=config.config.get('validate_a', False))
    ...
    if task_step > 0:
        ...
        best_source = 'test' if config.config.get('validate', False) else ('val' if config.config.get('validate_a', False) else None)
        old_path = _get_prev_step_checkpoint(prev_dir, config['trainer'], task_step - 1, best_source)
    ...
    if task_step > 0:
        if task_step ==1:
            lr_scheduler = config.init_obj(
                'lr_scheduler',
                module_lr_scheduler,
                **{"optimizer": optimizer, "max_iters": config["trainer"]['epochs_incre'] * len(train_loader)}
            )
        else:
            lr_scheduler = config.init_obj(
                'lr_scheduler',
    ...
    if config.config.get('validate_a', False):
