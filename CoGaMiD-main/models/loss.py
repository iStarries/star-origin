...
class WBCELoss(nn.Module):
    ...
    def forward(self, logit, label,fake_features=None):
        # logit:     [N, |Ct|, H, W]
        # label:     [N, H, W]
        N, C, H, W = logit.shape
...
class PKDLoss(nn.Module):
    ...
    def forward(self, features, features_old, pseudo_label_region,prev_fes_model=None,device=None):

        pseudo_label_region_5 = F.interpolate(
            pseudo_label_region, size=features[-1].shape[2:], mode="bilinear", align_corners=False)
        prob = []
        prob_old = []
        for cls in range(0, self.n_old_classes - 1):
            log_prob = prev_fes_model[cls].to(device).score_samples(features[-1].permute(0, 2, 3, 1).reshape(-1, 256))
            log_prob_old = prev_fes_model[cls].to(device).score_samples(features_old[-1].permute(0, 2, 3, 1).reshape(-1, 256))
            prob.append(log_prob.unsqueeze(1))
            prob_old.append(log_prob_old.unsqueeze(1))
        prob = torch.cat(prob, dim=1)
        prob_old = torch.cat(prob_old, dim=1)
        loss_5 = self.criterion(F.normalize(prob, p=2, dim=1), F.normalize(prob_old, p=2, dim=1))
        loss_6= self.criterion(features[-1], features_old[-1])
        pseudo_label_region_6 = pseudo_label_region_5.reshape(-1).unsqueeze(1)
        loss_5 = (loss_5 * pseudo_label_region_6).sum() / (
                    (pseudo_label_region_5.sum()*loss_5.shape[-1]) + 0.0001)
        loss_6 = (loss_6 * pseudo_label_region_5).sum() / (
...
class ContLoss(nn.Module):
    ...
    def forward(self, features, logit, label,n_new_classes,prev_gmm_prototypes=None):
        ...
        dist_pp = torch.norm(new_center.unsqueeze(0) - F.normalize(prev_gmm_prototypes, p=2, dim=1).unsqueeze(1), p=2,
                             dim=2)
